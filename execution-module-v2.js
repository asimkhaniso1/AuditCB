...
`;
    ...
    window.exportReportPDF = function () {
        ...
        const reportHtml = '<!DOCTYPE html><html><head>'
            ...
            + '@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact;padding-top:28mm;padding-bottom:18mm;font-size:10pt;}.page-break{page-break-before:always;padding-top:4mm;}.no-print{display:none !important;}.section-card,tr{break-inside:avoid;}.sh{-webkit-print-color-adjust:exact !important;print-color-adjust:exact !important;break-after:avoid;margin-top:10px;}.sb{break-before:avoid;}}'
            + '@media print{@page{size:A4;margin:25mm 12mm 22mm 12mm;@bottom-center{content:counter(page);font-size:0.7rem;color:#64748b;}}.rpt-hdr{display:flex !important;}.rpt-ftr{display:flex !important;}.cover{padding-top:0 !important;margin-top:-28mm;}.cover .rpt-hdr,.cover .rpt-ftr{display:none !important;}}'
            ...
            + '.rpt-hdr-left{display:flex;align-items:center;gap:8px;font-weight:700;font-size:0.82rem;color:#1e3a5f;}'
            + '.rpt-hdr-logo{height:22px;max-width:100px;object-fit:contain;border-radius:3px;}'
            ...
            + '.chart-grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-top:10px;}'
            + '.chart-box{background:white;border:1px solid #e2e8f0;border-radius:10px;padding:16px;text-align:center;}'
            + '.chart-box canvas{max-height:200px;}'
            + '.chart-title{font-size:0.78rem;font-weight:700;color:#334155;margin-bottom:8px;text-transform:uppercase;letter-spacing:0.3px;}'
            ...
            + (obsOnlyRowsHtml ? '<div id="sec-obs" class="sh page-break" style="border-left-color:#7c3aed;"><span class="sn">5</span>OBSERVATIONS</div><div class="sb" style="padding:0;"><table class="f-tbl"><thead><tr style="background:#f5f3ff;"><th style="width:12%;">Clause</th><th style="width:28%;">ISO Requirement</th><th style="width:12%;text-align:center;">Type</th><th style="width:48%;">Details</th></tr></thead><tbody>' + obsOnlyRowsHtml + '</tbody></table></div>' : '')
            // SECTION 6 - OPPORTUNITIES FOR IMPROVEMENT
            + (ofiOnlyRowsHtml ? '<div id="sec-ofi" class="sh page-break" style="border-left-color:#06b6d4;"><span class="sn">6</span>OPPORTUNITIES FOR IMPROVEMENT</div><div class="sb" style="padding:0;"><table class="f-tbl"><thead><tr style="background:#ecfeff;"><th style="width:12%;">Clause</th><th style="width:28%;">ISO Requirement</th><th style="width:12%;text-align:center;">Type</th><th style="width:48%;">Recommendation</th></tr></thead><tbody>' + ofiOnlyRowsHtml + '</tbody></table></div>' : '')
            // SECTION 7 - NON-CONFORMITY DETAILS
            + (en['findings'] !== false ? '<div id="sec-findings" class="sh page-break" style="border-left-color:#dc2626;"><span class="sn">7</span>NON-CONFORMITY DETAILS</div><div class="sb" style="padding:0;"><table class="f-tbl"><thead><tr><th style="width:12%;">Clause</th><th style="width:28%;">ISO Requirement</th><th style="width:12%;text-align:center;">Severity</th><th style="width:48%;">Evidence & Remarks</th></tr></thead><tbody>' + (ncRowsHtml || '<tr><td colspan="4" style="padding:24px;text-align:center;color:#94a3b8;">No non-conformities found.</td></tr>') + '</tbody></table></div>' : '')
            // SECTION 8 - NCR REGISTER
            + (en['ncrs'] !== false && (d.report.ncrs || []).length > 0 ? '<div id="sec-ncrs" class="sh page-break" style="border-left-color:#ea580c;"><span class="sn">8</span>NCR REGISTER</div><div class="sb">' + d.report.ncrs.map(ncr => '<div style="padding:14px 18px;border-left:4px solid ' + (ncr.type === 'Major' ? '#dc2626' : '#f59e0b') + ';background:' + (ncr.type === 'Major' ? '#fef2f2' : '#fffbeb') + ';border-radius:0 8px 8px 0;margin-bottom:12px;"><div style="display:flex;justify-content:space-between;align-items:center;"><strong style="font-size:0.95rem;">' + ncr.type + ' — Clause ' + ncr.clause + '</strong><span style="color:#64748b;font-size:0.82rem;">' + (ncr.createdAt ? new Date(ncr.createdAt).toLocaleDateString() : '') + '</span></div><div style="color:#334155;font-size:0.9rem;margin-top:8px;line-height:1.7;">' + fmtRemark(ncr.description) + '</div>' + (ncr.evidenceImage ? '<div style="margin-top:8px;"><img src="' + ncr.evidenceImage + '" style="max-height:120px;border-radius:6px;border:1px solid #e2e8f0;"></div>' : '') + '</div>').join('') + '</div>' : '')
            // SECTION 9 - MEETINGS
            + (en['meetings'] !== false ? '<div id="sec-meetings" class="sh page-break" style="border-left-color:#0891b2;"><span class="sn">9</span>MEETING RECORDS</div><div class="sb"><div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">'
